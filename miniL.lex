...
{IDENTIFIER} {col += yyleng; char * token = new char[yyleng]; strcpy(token, yytext); yylval.op_val = token; return IDENTIFIER;} 
{DIGIT}+     {col += yyleng; char * token = new char[yyleng]; strcpy(token, yytext); yylval.op_val = token; return NUMBER;} 
